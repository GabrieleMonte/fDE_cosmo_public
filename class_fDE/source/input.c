...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (flag1 == _TRUE_) {
      ...
      else if ((strcmp(string1,"fpDE") == 0) || (strcmp(string1,"fpde") == 0)) {
	      pba->fluid_equation_of_state = fpDE;
      }
      else if ((strcmp(string1,"faDE_2") == 0) || (strcmp(string1,"fade_2") == 0)) {
	      pba->fluid_equation_of_state = faDE2;
      }
      else if ((strcmp(string1,"fpDE_2") == 0) || (strcmp(string1,"fpde_2") == 0)) {
	      pba->fluid_equation_of_state = fpDE2;
      }
    ...
    if (pba->fluid_equation_of_state == faDE) {
	    /* Read */
	    class_read_double("fa_fld",pba->fa_fld);
	    class_read_double("cs2_fld",pba->cs2_fld);
    }
    if (pba->fluid_equation_of_state == fpDE) {
	    /* Read */
	    class_read_double("fp_fld",pba->fp_fld);
	    class_read_double("ap_fld",pba->ap_fld);
	    class_read_double("cs2_fld",pba->cs2_fld);
    }
     if (pba->fluid_equation_of_state == faDE2) {
      	     /* Read */
	     class_read_double("fa_fld",pba->fa_fld);
	     class_read_double("dfa_fld",pba->dfa_fld);
	     class_read_double("cs2_fld",pba->cs2_fld);
     }
     if (pba->fluid_equation_of_state == fpDE2) {
	     /* Read */
	     class_read_double("fp_fld",pba->fp_fld);
	     class_read_double("wp_fld",pba->wp_fld);
	     class_read_double("ap_fld",pba->ap_fld);
	     class_read_double("cs2_fld",pba->cs2_fld);
     }
  ...
  if (pba->Omega0_scf != 0.){
      ...
      /** 8.b.0) Read potential name and map to type */
      class_alloc(pba->scf_potential_name, _SCF_NAME_MAX_LEN_ * sizeof(char), errmsg);
      class_read_string("scf_potential", pba->scf_potential_name);

      if (strcmp(pba->scf_potential_name, "hilltop") == 0)
        pba->scf_potential_type = 0;
      else if (strcmp(pba->scf_potential_name, "plateau") == 0)
        pba->scf_potential_type = 1;
      else if (strcmp(pba->scf_potential_name, "exponential") == 0)
        pba->scf_potential_type = 2;
      else {
        class_stop(errmsg,
          "Unrecognized scf_potential = '%s'. Choose one of: 'hilltop', 'plateau', or 'exponential'.",
          pba->scf_potential_name);
      }
    ...
    /* Declare all possible inputs */
    double param4, param5, param6, param7;
    int flag4, flag5, flag6, flag7;

    if (strcmp(pba->scf_potential_name, "hilltop") == 0) {

      // Expect: Lambda0_scf (V0), K_scf (k), phi_i_scf
      class_call(parser_read_double(pfc,"Lambda0_scf",&param4,&flag4,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"K_scf",&param5,&flag5,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"phi_i_scf",&param6,&flag6,errmsg),errmsg,errmsg);

      if (flag4 == _FALSE_ || flag5 == _FALSE_ || flag6 == _FALSE_) {
        class_stop(errmsg, "Missing one or more required hilltop parameters: Lambda0_scf, K_scf, phi_i_scf");
      }

      if (pba->scf_parameters_size < 3) {
        class_stop(errmsg, "Hilltop model requires at least 3 scf_parameters (V0, K, phi_i)");
      }

      pba->scf_parameters[1] = param4; // V0
      pba->scf_parameters[2] = param5; // K
      pba->scf_parameters[pba->scf_parameters_size - 2] = param6; // phi_i

    }
    else if (strcmp(pba->scf_potential_name, "plateau") == 0) {

      // Expect: Lambda0_scf (V0), M_scf, m_scf, kappa_scf, phi_i_scf
      class_call(parser_read_double(pfc,"Lambda0_scf",&param4,&flag4,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"M_scf",&param5,&flag5,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"m_scf",&param6,&flag6,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"kappa_scf",&param7,&flag7,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"phi_i_scf",&param6,&flag6,errmsg),errmsg,errmsg); // reusing param6 for phi_i

      if (flag4 == _FALSE_ || flag5 == _FALSE_ || flag6 == _FALSE_ || flag7 == _FALSE_) {
        class_stop(errmsg, "Missing one or more required plateau parameters: Lambda0_scf, M_scf, m_scf, kappa_scf, phi_i_scf");
      }

      if (pba->scf_parameters_size < 5) {
        class_stop(errmsg, "Plateau model requires at least 5 scf_parameters (V0, M, m, kappa, phi_i)");
      }

      pba->scf_parameters[1] = param4; // V0
      pba->scf_parameters[2] = param5; // M
      pba->scf_parameters[3] = param6; // m
      pba->scf_parameters[4] = param7; // kappa
      pba->scf_parameters[pba->scf_parameters_size - 2] = param6; // phi_i

    }
    else if (strcmp(pba->scf_potential_name, "exponential") == 0) {

      // Expect: Lambda0_scf (V0), lambda_scf, phi_i_scf
      class_call(parser_read_double(pfc,"Lambda0_scf",&param4,&flag4,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"lambda_scf",&param5,&flag5,errmsg),errmsg,errmsg);
      class_call(parser_read_double(pfc,"phi_i_scf",&param6,&flag6,errmsg),errmsg,errmsg);

      if (flag4 == _FALSE_ || flag5 == _FALSE_ || flag6 == _FALSE_) {
        class_stop(errmsg, "Missing one or more required exponential parameters: Lambda0_scf, lambda_scf, phi_i_scf");
      }

      if (pba->scf_parameters_size < 3) {
        class_stop(errmsg, "Exponential model requires at least 3 scf_parameters (V0, lambda, phi_i)");
      }

      pba->scf_parameters[1] = param4; // V0
      pba->scf_parameters[2] = param5; // lambda
      pba->scf_parameters[pba->scf_parameters_size - 2] = param6; // phi_i
    }
    else {
      class_stop(errmsg, "Unknown potential type %s. You must choose hilltop, plateau, or exponential.", pba->scf_potential_name);
    }
    ...
    // Update scf_params inputs  
    pba->scf_parameters[1] = param4;
    pba->scf_parameters[2] = param5;
    pba->scf_parameters[pba->scf_parameters_size-2] = param6;
    */
