...
int background_w_fld(
  ...
  case faDE:
    *integral_fld = 1. + pba->fa_fld*(1-a);
    break;
  case fpDE:
    *integral_fld = (a-pba->ap_fld)/(1-pba->ap_fld) + pba->fp_fld*(1-a)/(1-pba->ap_fld);
    break;
  case faDE2:
    *integral_fld = 1. + pba->fa_fld*(1-a)+ pba->dfa_fld*pow((1-a),2);
    break;
  case fpDE2:
    *integral_fld = pba->fp_fld -(3*(1+pba->wp_fld)*pba->fp_fld)/pba->ap_fld*(a-pba->ap_fld)+((1-pba->fp_fld+(3*(1+pba->wp_fld)*pba->fp_fld)/pba->ap_fld*(1-pba->ap_fld))/pow((1-pba->ap_fld),2))*pow((a-pba->ap_fld),2);
    break;
  case EDE:
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    if (pba->fluid_equation_of_state == faDE  || pba->fluid_equation_of_state == fpDE  || 
    pba->fluid_equation_of_state == faDE2 || pba->fluid_equation_of_state == fpDE2) {
		pvecback_integration[pba->index_bi_rho_fld] = rho_fld_today * integral_fld;
		}
	else {
...
int background_derivs(
  ...
  if (pba->has_fld == _TRUE_) {
	if (pba->fluid_equation_of_state == fpDE) {  
		dy[pba->index_bi_rho_fld] = -((pba->fp_fld-1)/(1-pba->ap_fld))*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else if (pba->fluid_equation_of_state == faDE) {  
		dy[pba->index_bi_rho_fld] = -pba->fa_fld*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else if (pba->fluid_equation_of_state == fpDE2) {
		double fb_at_a = (1 - pba->fp_fld + (3*(1+pba->wp_fld)*pba->fp_fld/pba->ap_fld)*(1-pba->ap_fld)) / pow((1-pba->ap_fld),2);
		double dfDE_at_a	= -3*(1+pba->wp_fld)*pba->fp_fld/pba->ap_fld+2*fb_at_a*(a-pba->ap_fld);
		dy[pba->index_bi_rho_fld] = dfDE_at_a*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else if (pba->fluid_equation_of_state == faDE2) {  
		dy[pba->index_bi_rho_fld] = -(pba->fa_fld+2*pba->dfa_fld*(1-a))*pba->Omega0_fld * pow(pba->H0,2)*a;
	}
	else {
...
/*
	...
	Add in code to do hilltop potential
	
	-- numerical factors below to get class units
...
double V_scf(
  ...
  double scf_K   = pba->scf_parameters[2];
  return  3.968e-8*scf_V0*(1. - 0.5 * pow(scf_K,2.)*pow(phi,2.));
}
...
double dV_scf(
  ...
  double scf_V0  = pba->scf_parameters[1];
  double scf_K   = pba->scf_parameters[2];
  return -1. * 3.968e-8*scf_V0*pow(scf_K,2.)*phi;
}
...
double ddV_scf(
  ...
  double scf_V0  = pba->scf_parameters[1];
  double scf_K   = pba->scf_parameters[2];
  return -1 * 3.968e-8*scf_V0*pow(scf_K,2.);
}
*/


double V_scf(struct background *pba, double phi) {
  double V0 = pba->scf_parameters[1];
  int pot_type = pba->scf_potential_type;

  if (pot_type == 0) {
    // Hilltop: V(phi) = V0 * (1 - 0.5 * k^2 * phi^2)
    double K = pba->scf_parameters[2];
    return 3.968e-8 * V0 * (1. - 0.5 * K * K * phi * phi);
  }

  else if (pot_type == 1) {
    // Plateau: V(phi) = V0 * (exp(-phi / M) - kappa * exp(phi / m))
    double M     = pba->scf_parameters[2];
    double m     = pba->scf_parameters[3];
    double kappa = pba->scf_parameters[4];
    return 3.968e-8 * V0 * (exp(-phi / M) - kappa * exp(phi / m));
  }

  else if (pot_type == 2) {
    // Exponential: V(phi) = V0 * exp(lambda * phi)
    double lambda = pba->scf_parameters[2];
    return 3.968e-8 * V0 * exp(lambda * phi);
  }
...
double dV_scf(struct background *pba, double phi) {
  ...
  if (pot_type == 0) {
    // dV/dphi = -V0 * k^2 * phi
    double K = pba->scf_parameters[2];
    return -3.968e-8 * V0 * K * K * phi;
  }

  else if (pot_type == 1) {
    // dV/dphi = V0 * [-1/M * exp(-phi / M) - kappa/m * exp(phi / m)]
    double M     = pba->scf_parameters[2];
    double m     = pba->scf_parameters[3];
    double kappa = pba->scf_parameters[4];
    return 3.968e-8 * V0 * (-1. / M * exp(-phi / M) - kappa / m * exp(phi / m));
  }

  else if (pot_type == 2) {
    // dV/dphi = lambda * V0 * exp(lambda * phi)
    double lambda = pba->scf_parameters[2];
    return 3.968e-8 * V0 * lambda * exp(lambda * phi);
  }
...
double ddV_scf(struct background *pba, double phi) {
  ...
  if (pot_type == 0) {
    // d²V/dphi² = -V0 * k^2
    double K = pba->scf_parameters[2];
    return -3.968e-8 * V0 * K * K;
  }

  else if (pot_type == 1) {
    // d²V/dphi² = V0 * [1/M² * exp(-phi/M) - kappa/m² * exp(phi/m)]
    double M     = pba->scf_parameters[2];
    double m     = pba->scf_parameters[3];
    double kappa = pba->scf_parameters[4];
    return 3.968e-8 * V0 * (1. / (M * M) * exp(-phi / M) - kappa / (m * m) * exp(phi / m));
  }

  else if (pot_type == 2) {
    // d²V/dphi² = lambda² * V0 * exp(lambda * phi)
    double lambda = pba->scf_parameters[2];
    return 3.968e-8 * V0 * lambda * lambda * exp(lambda * phi);
  }
...
double V_scf(
             struct background *pba,
             double phi) {
  return  V_hilltop_scf(pba,phi);
  //V_e_scf(pba,phi)*V_p_scf(pba,phi);
}

double dV_scf(
              struct background *pba,
              double phi) {
  return dV_hilltop_scf(pba,phi);
  //dV_e_scf(pba,phi)*V_p_scf(pba,phi) + V_e_scf(pba,phi)*dV_p_scf(pba,phi);
}

double ddV_scf(
               struct background *pba,
               double phi) {
  return ddV_hilltop_scf(pba,phi);
  //ddV_e_scf(pba,phi)*V_p_scf(pba,phi) + 2*dV_e_scf(pba,phi)*dV_p_scf(pba,phi) + V_e_scf(pba,phi)*ddV_p_scf(pba,phi);
}
*/
